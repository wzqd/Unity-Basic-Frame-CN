...
public abstract class BaseFSM: MonoBehaviour
{
    ...
    public Enum CurrentStateType { get; set; } //枚举类型,可以设置一个初始状态
    private BaseState currentState;


    /// <summary>
    /// 虚方法 生命周期函数 使用时记得加override关键字
    /// </summary>
    ...
    {
        if (newState == CurrentStateType) return; //如果重复进入同一个状态，直接返回
        if(currentState != null) currentState.Exit(); //执行退出当前状态的方法
